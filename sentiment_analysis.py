    ...
    for test_case in test_cases:
        ...
        try:
            if test_case['name'] == "simple_test_check":
                assert test_case["expected"] == tweet_to_tensor(
                    *test_case['input'])
                count += 1
            if test_case['name'] == "datatype_check":
                assert isinstance(tweet_to_tensor(
...
### END CODE HERE ###
    ...
    def init_weights_and_state(self, input_signature, random_key):
        ...
        w = random.normal(key=random_key, shape=(
            input_shape[-1], self._n_units)) * self._init_stdev
...
training_loop = train_model(
    model, train_task, eval_task, 100, output_dir_expand)
...
print(
    f"The shape of the labels is {tmp_targets.shape}, which is the batch size.")
print(
...
print(
    f"The prediction shape is {tmp_pred.shape}, num of tensor_tweets as rows")
...
print(
    f"Model's prediction accuracy on a single training batch is: {100 * tmp_acc}%")
print(
...
sentence = "Hey, this is pretty good!"
tmp_pred, tmp_sentiment = predict(sentence)
print(
    f"The sentiment of the sentence \n***\n\"{sentence}\"\n***\nis {tmp_sentiment}.")
